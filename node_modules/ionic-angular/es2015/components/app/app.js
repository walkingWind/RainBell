...
import { DIRECTION_FORWARD, DIRECTION_BACK, isTabs } from '../../navigation/nav-util';
...
export class App {
    ...
    navPop() {
        ...
        this._rootNavs.forEach((navContainer) => {
            const /** @type {?} */ activeNav = this.getActiveNav(navContainer.id);
            const /** @type {?} */ poppable = getPoppableNav(activeNav);
            if (poppable) {
...
function findTopNav(nav) {
    while (nav) {
        const /** @type {?} */ childNav = nav.getActiveChildNav();
        if (!childNav) {
            break;
        }
        nav = childNav;
